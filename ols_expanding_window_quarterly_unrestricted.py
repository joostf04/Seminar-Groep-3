...
fig.tight_layout()
fig.savefig("plot_S_fr_combined_quarterly_unrestricted.png", dpi=150)
plt.close(fig)
...
ax.grid(True, alpha=0.3)
fig.tight_layout()
fig.savefig("plot_S_p_combined_quarterly_unrestricted.png", dpi=150)
plt.close(fig)
...
fig.tight_layout()
fig.savefig("plot_pred_vs_eqprem_1994_2000_quarterly_unrestricted.png", dpi=150)
plt.close(fig)
...
for idx, W in enumerate(WINDOWS):
    ...
    ax.plot(np.array(dates_oos)[_mask], _rv[_mask],
            ...
            linewidth=1.2, label=f"W = {W} quarters")
...
for W in WINDOWS:
    ...
    ax.set_title(f"Individual Predictor Return Correlations (W = {W} quarters)",
                 fontsize=13)
    ax.tick_params(axis="both", labelsize=11)
    ax.xaxis.set_major_locator(mdates.YearLocator(5))
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
