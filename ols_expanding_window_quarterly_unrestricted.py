...
df["date"] = pd.to_datetime(
    _year.astype(str) + _month.astype(str).str.zfill(2) + "01",
    format="%Y%m%d"
)
...
ANN         = 4           # quarterly annualisation
WINDOWS     = [20, 40, 60]  # 20, 40, 60 quarters
VAR_WINDOW  = 40           # 40 quarters = 10 years

df = df.sort_values("date").reset_index(drop=True)
print(f"Quarterly | Annualisation: x{ANN} | Rolling windows: {WINDOWS}")

# ── Define predictors ──────────────────────────────────────────────────────────
PREDICTORS = [c for c in df.columns if c not in ("yyyyq", "date", "eqprem", "simple_eqprem", "RETURNS INCL DIV", "RF")]
print(f"Predictors ({len(PREDICTORS)}): {PREDICTORS}\n")
print(f"\n=== QUARTERLY DATA DIAGNOSTIC ===")
...
# ── Split boundary ─────────────────────────────────────────────────────────────
IN_SAMPLE_END   = pd.Timestamp("1978-10-01")   # last in-sample quarter (1978 Q4)
OOS_START_PRED  = pd.Timestamp("1979-01-01")   # first quarter we *predict* (1979 Q1)
OOS_END_PRED    = pd.Timestamp("2022-10-01")   # last quarter we *predict* (2022 Q4)
...
oos_index = df.index[oos_mask].tolist()
oos_mask_eval = (df["date"] >= OOS_START_PRED) & (df["date"] <= OOS_END_PRED)
oos_index_eval = df.index[oos_mask_eval].tolist()
...
for pred in PREDICTORS:
    ...
    lag_col = f"{pred}_lag"
    data[lag_col] = data[pred].shift(1)
    data = data.dropna(subset=[lag_col])
...
print(f"  OOS obs: {len(combined)} | Avg predictors per quarter: {n_preds_used:.1f}")

# ── Save results ───────────────────────────────────────────────────────────────
out_path2 = "ols_oos_predictions_quarterly_unrestricted.xlsx"
...
actual_vec  = combined["eqprem"].values
pred_matrix = combined[forecast_cols].values
...
sfr_results  = {}
sp_results   = {}
rho_rows = {W: [] for W in WINDOWS}
rho_rows_60q = rho_rows[WINDOWS[-1]]   # 60 quarters = 15 years
rho_rows_40q = rho_rows[WINDOWS[-2]]   # 40 quarters = 10 years
rho_rows_20q = rho_rows[WINDOWS[-3]]   # 20 quarters =  5 years

for W in WINDOWS:
    print(f"  Window = {W} quarters ...")
    rows_sfr  = []
    rows_sp   = []
    ...
    for t in range(2, T + 1):
        ...
        P_ranked = np.apply_along_axis(rankdata, 0, P_window)
        corr_mat = np.corrcoef(P_ranked.T)
...
with pd.ExcelWriter(out_path2, engine="openpyxl", mode="a",
                    if_sheet_exists="replace") as writer:
    pd.DataFrame(rho_rows[20]).to_excel(writer, sheet_name="Rho_20q", index=False)
    pd.DataFrame(rho_rows[40]).to_excel(writer, sheet_name="Rho_40q", index=False)
    pd.DataFrame(rho_rows[60]).to_excel(writer, sheet_name="Rho_60q", index=False)
...
_bestaic_pred   = []
_aic_weight_log = []
_oos_ss = {K: 0.0 for K in ALL_K}
_oos_n  = 0

print(f"  Fitting PCA({MAX_K}) once per OOS quarter; nested factor sets 1..K ...")
...
for oos_row in oos_index:
    ...
    F_tr  = pca.fit_transform(X_tr_scaled)
    ...
    F_oos = pca.transform(X_oos * slopes)
...
print("\n" + "=" * 60)
print("Table 2: AIC and BIC for direct sPCA specifications (Quarterly)")
print("=" * 60)

_AIC_P1  = pd.Timestamp("1979-01-01")
_AIC_PE  = pd.Timestamp("2022-10-01")
_AIC_P2  = pd.Timestamp("1994-01-01")
_AIC_P93 = pd.Timestamp("1993-10-01")
...
FULL_END     = pd.Timestamp("2022-10-01")
PRE94_END    = pd.Timestamp("1993-10-01")
...
def clark_west(actual: np.ndarray, predicted: np.ndarray, benchmark: np.ndarray):
    ...
    p_value = scipy_stats.norm.sf(t_stat)
...
for idx, W in enumerate(WINDOWS):
    ...
    ax.plot(_df["date"], _df["S_fr"],
            ...
            linewidth=1.3, label=f"W={W} quarters")
...
fig.tight_layout()
fig.savefig("plot_S_fr_combined_quarterly_unrestricted.png", dpi=300, bbox_inches="tight")
plt.close(fig)
print("  Saved: plot_S_fr_combined_quarterly_unrestricted.png")
...
ax.grid(True, alpha=0.3)
fig.tight_layout()
fig.savefig("plot_S_p_combined_quarterly_unrestricted.png", dpi=300, bbox_inches="tight")
plt.close(fig)
print("  Saved: plot_S_p_combined_quarterly_unrestricted.png")
...
_PLOT_END   = pd.Timestamp("2000-10-01")
_PRE94_END  = pd.Timestamp("1993-10-01")
...
_roll_q = 4
_plot_df["avg_pred_roll"] = _plot_df["avg_pred"].rolling(_roll_q, min_periods=1).mean()
_plot_df["eqprem_roll"]   = _plot_df["eqprem"].rolling(_roll_q, min_periods=1).mean()
...
ax1.plot(_plot_df["date"], _plot_df["avg_pred_roll"],
         color=color_pred, linewidth=2.0, linestyle="-", label="Avg. predictor (4q avg)")
...
ax2.plot(_plot_df["date"], _plot_df["eqprem_roll"],
         color=color_eq, linewidth=2.0, alpha=0.9, linestyle="--", dashes=[6, 3], label="Equity premium (4q avg)")
...
fig.tight_layout()
fig.savefig("plot_pred_vs_eqprem_1994_2000_quarterly_unrestricted.png", dpi=300, bbox_inches="tight")
plt.close(fig)
print("  Saved: plot_pred_vs_eqprem_1994_2000_quarterly_unrestricted.png")
...
_dm_P = _dm_oos[_fc_arr].values.astype(float)
_dm_r = _dm_oos["eqprem"].values
...
DOLS_DELTAS = [0.96594, 0.98282, 0.98851]
...
for _dl in DOLS_DELTAS:
    ...
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    _fname = f"plot_DOLS_S_fr_delta{int(round(_dl*100000))}_quarterly_unrestricted.png"
...
ax.grid(True, alpha=0.3)
fig.tight_layout()
_fname_png = "plot_return_variance_quarterly_unrestricted.png"
_fname_pdf = "plot_return_variance_quarterly_unrestricted.pdf"
...
for W in WINDOWS:
    ...
    _fname_png = f"individual_predictor_corr_quarterly_unrestricted_W{W}.png"
    _fname_pdf = f"individual_predictor_corr_quarterly_unrestricted_W{W}.pdf"
    fig.savefig(_fname_png, dpi=300, bbox_inches="tight")
    fig.savefig(_fname_pdf, bbox_inches="tight")
    plt.close(fig)
    print(f"    Saved: {_fname_png} + {_fname_pdf}")
