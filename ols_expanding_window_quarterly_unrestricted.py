...
WINDOWS     = [20, 40, 60]  # 60, 120, 180 months
VAR_WINDOW  = 40           # 40 quarters = 10 years
_VAR_PLOT_W = [20, 40, 60]  # 60, 120, 180 months
...
# Named aliases for the three windows (used in rho plot)
...
_PRE94  = pd.Timestamp("1993-10-01")
...
DEC2005_END  = pd.Timestamp("2005-10-01")
...
GAMMA      = 3.0
# VAR_WINDOW = 40 quarters (10 years)
W_MIN, W_MAX = 0.0, 1.5
N_BOOT     = 1000
CI_LEVEL   = 0.90
...
_roll_q = 4  # 4-quarter rolling average
...
for _dl in DOLS_DELTAS:
    ...
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    _fname = f"plot_DOLS_S_p_delta{int(round(_dl*100000))}_quarterly_unrestricted.png"
    fig.savefig(_fname, dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"    Saved: {_fname}")
